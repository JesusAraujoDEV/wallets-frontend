...
export default function Login({ onSuccess, customTitle, hideNavigation }: LoginProps) {
  ...
  const handleGoogleSuccess = createGoogleLoginHandler({ setLoading, onSuccess, toast });
  const onSubmit = createSubmitHandler({
    values: { isLogin, name, email, usernameOrEmail, password },
    setLoading, setFieldErrors, toast, onSuccess,
  });
        ...
        <div className="w-full max-w-md bg-card text-card-foreground p-8 rounded-2xl shadow-xl lg:shadow-none">
          ...
          <div className="mb-6 flex justify-center">
            <LoginGoogleButton
              ...
              onError={() => toast({ title: "Google Login fallido", description: "No fue posible autenticar con Google", variant: "destructive" })}
            />
          </div>
