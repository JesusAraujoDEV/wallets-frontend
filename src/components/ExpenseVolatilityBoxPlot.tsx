...
export interface ExpenseVolatilityBoxPlotProps {
  categories: Array<{ category: string; min: number; q1: number; median: number; q3: number; max: number; outliers?: number[]; count?: number }>;
}

export function ExpenseVolatilityBoxPlot({ categories }: ExpenseVolatilityBoxPlotProps) {
  ...
  return (
    <Card className="p-6 shadow-md border-0">
      <h3 className="text-xl font-semibold text-foreground mb-4">Expense Volatility (Box Plot)</h3>
        ...
        <svg width={width} height={height}>
          ...
          {categories.map((c, i) => {
            ...
            return (
              <g key={i}>
                {/* whiskers */}
                <line x1={x + w/2} x2={x + w/2} y1={yMin} y2={yQ1} stroke="hsl(var(--foreground))" />
                <line x1={x + w/2} x2={x + w/2} y1={yQ3} y2={yMax} stroke="hsl(var(--foreground))" />
                <line x1={x + w*0.25} x2={x + w*0.75} y1={yMin} y2={yMin} stroke="hsl(var(--foreground))" />
                <line x1={x + w*0.25} x2={x + w*0.75} y1={yMax} y2={yMax} stroke="hsl(var(--foreground))" />
                {/* box */}
                <rect x={x} y={Math.min(yQ1,yQ3)} width={w} height={Math.abs(yQ3 - yQ1)} fill="hsl(var(--primary)/0.15)" stroke="hsl(var(--primary))" />
                {/* median */}
                <line x1={x} x2={x + w} y1={yMed} y2={yMed} stroke="hsl(var(--primary))" strokeWidth={2} />
                {/* outliers */}
                {(c.outliers || []).map((o, j) => (
                  <circle key={j} cx={x + w/2} cy={scaleY(o)} r={3} fill="hsl(var(--accent))">
