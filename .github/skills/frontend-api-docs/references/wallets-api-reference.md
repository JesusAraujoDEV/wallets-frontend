...
Purpose: List all recurring transactions configured by the user.
...
Purpose: Create a new recurring transaction template.
...
Note: `accountId` is optional (nullable). `currency` defaults to `USD` if omitted. Valid frequencies: `weekly` | `monthly` | `yearly`. Valid modes: `auto` | `manual`.

### PATCH /recurring-transactions/:id
Purpose: Update a recurring transaction (e.g., pause it).
...
Purpose: Delete a recurring transaction.

### POST /recurring-transactions/trigger
Purpose: Manually execute the cronjob/worker for the authenticated user to process due subscriptions immediately (testing/dev).
...
Note: Transactions now include a `status` field with values `pending` or `completed`.
...
Purpose: Confirm a pending transaction generated by a manual recurring subscription.
...
Note: `date`, `amount`, and `currency` are optional defaults. `accountId` is strictly required to determine where the money was deducted from. On success, `status` changes from `pending` to `completed`.
