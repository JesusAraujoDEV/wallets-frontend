# AGENTS.md — wallets-frontend IA Entry
...
React/Vite frontend for Wallets, a personal finance app (pastel-wallet-insights on GitHub). Consumes the API in the sibling repo `wallets-backend`; no shared git history between them.
...
| Layer | Tech |
|-------|------|
| Build | Vite 5, TypeScript 5, SWC (`@vitejs/plugin-react-swc`) |
| UI | React 18, shadcn/ui (Radix primitives), MUI (date pickers), Tailwind CSS + tailwindcss-animate |
| Data/state | @tanstack/react-query, react-hook-form + @hookform/resolvers, zod |
| Routing | react-router-dom |
| Auth | jose, @react-oauth/google, bcryptjs |
| Charts | recharts |
| Lint | ESLint 9 (flat config) + typescript-eslint |

Locked decisions (no re-discuss): no Redux (react-query + local state instead), no GraphQL, shadcn/ui as the component base (don't hand-roll primitives Radix already covers), Zod as the single validation layer at API boundaries.

## Architecture rules

- **Layered flow**: Pages (`src/pages`) → Hooks (`src/hooks`) → lib/services (`src/lib`) → Components (`src/components`).
- **No business logic in views.** Components render props/state and dispatch events; data fetching lives in hooks via react-query.
- **Single source of truth for types**: Zod schemas at every API boundary; infer TS types from them, don't hand-duplicate.
- `src/components/ui` is the shadcn primitive layer — extend via composition, don't fork generated files unless upgrading shadcn itself.
...
src/
  components/
    ...
    __tests__/
  hooks/        # data fetching (react-query) + view logic
